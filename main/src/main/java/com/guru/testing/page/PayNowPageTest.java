...
public class PayNowPageTest {
	...
	public static void verifyPayNowPageTest() throws Exception {
		...
		try {
			...
			throw new HTMLElementNotFoundException(e, "Required elements to Verify Pay now page not found.");
		}
	}
	...
	public static void uncheckAutoUpgradeIfCheckedTest() throws Exception {
		...
		try {
			...
			if(BrowserAction.getElement(PayNowPageObjectMap.PAY_NOW_PAGE_AUTO_RENEW_ONE_CHECKBOX_ID).isSelected()){
				BrowserAction.click(PayNowPageObjectMap.PAY_NOW_PAGE_AUTO_RENEW_ONE_CHECKBOX_ID);
			}
			verifyAutoUpgradeMembershipIsUncheckedTest();
		}
	}
	...
	public static void verifyMembershipInvoiceTest(String mem) throws Exception {
		...
		try {
			...
			BrowserWait.waitUntilText("Expires on");
			BrowserWait.waitUntilText("Thank you for your purchase! We sent a copy of your receipt to ");
	...
	@Test
	@Documentation(step = "Click 'Back to Membership page' link in the payment receipt place", expected = "Able to click.")
	public static void clickBacktoMembershipTest() throws Exception {
		ScriptLogger.info();
		try {
			BrowserAction.click(PayNowPageObjectMap.PAY_NOW_PAGE_BACK_TO_MEMBERSHIP_PAGE_PLINK);			
		} catch (Exception e) {
			throw new ApplicationException(e, "Unable to click on 'Back to Membership page' link on the Payment receipt.");
		}
	}
	...
	public static void clickPayWithCCTest() throws Exception {
		...
		try {
			...
			BrowserWait.waitUntilElementIsDisplayed(PaymentsTransferPageObjectMap.PAYMENTS_PAGE_TRANSFER_METHODS_CREDIT_CARD_POPUP_CARD_NUMBER_TEXTBOX_XPATH, 10);
		} catch (Exception e) {
			throw new ApplicationException(e, "Unable to click on 'Pay with Credit Card'. Or possible that the Credit card form didn't appear after clicking.");
		}
	}
	...
	public static void enterCCNumberTest(String number) throws Exception {
		ScriptLogger.info();
		PaymentsTransferPageTest.enterCCNumberTest(number);
	}
	...
	public static void verifyBidPurchaseSuccessTest(int bidsAdded) throws Exception {
		...
		try {
			...
			throw new HTMLElementNotFoundException(e, "One or more of the elements required for verification/validation of successful bids purchase wasn't successful.");
		}
	}
	...
	public static void selectPaypalTest() throws Exception {
		...
		try {
			...
			case "American Express - 0005":
			case "American Express - 8431":
				BrowserAction.click(PayNowPageObjectMap.PAY_NOW_PAGE_INVOICE_PAYMENT_SELECT_PAYMENT_METHOD_CHANGE_BUTTON_XPATH);
				BrowserWait.waitUntilElementIsDisplayed(PayNowPageObjectMap.PAY_NOW_PAGE_SELECT_YOUR_PAYMENT_METHOD_CONTAINER_APPEARING_ON_CLICKING_CHANGE_XPATH);
				List<WebElement> thElements = BrowserAction.getElements(PayNowPageObjectMap.PAY_NOW_PAGE_PAYMENT_METHOD_ROWS_APPEARING_ON_CLICKING_CHANGE_XPATH);
				int index = 0;
	...
	@Test
	@Documentation(step = "Select ECheck as payment method.", expected = "Able to Select Credit card.")
	public static void selectECheckTest() throws Exception {
		ScriptLogger.info();
		try {
			String method = BrowserAction.getElement(PayNowPageObjectMap.PAY_NOW_PAGE_SELECT_A_PAYMENT_SELECTED_METHOD_NAME_XPATH).getText();
			if(method.contains("US BANK")) {
				ScriptLogger.info("ECheck is primary");
			}
			else if(method.contains("gurucomdev_biz@gmail.com") || method.contains("gurucomdev_per@gmail.com")) {
				BrowserAction.click(PayNowPageObjectMap.PAY_NOW_PAGE_INVOICE_PAYMENT_SELECT_PAYMENT_METHOD_CHANGE_BUTTON_XPATH);
				BrowserWait.waitUntilElementIsDisplayed(PayNowPageObjectMap.PAY_NOW_PAGE_SELECT_YOUR_PAYMENT_METHOD_CONTAINER_APPEARING_ON_CLICKING_CHANGE_XPATH);
				List<WebElement> thElements = BrowserAction.getElements(PayNowPageObjectMap.PAY_NOW_PAGE_PAYMENT_METHOD_ROWS_APPEARING_ON_CLICKING_CHANGE_XPATH);
				int index=0;
				...
				for(WebElement el : thElements) {
					if(el.findElement(By.tagName("p")).getText().contains("US BANK")) {
						break;
					} else {
						index++;
					}
			...
			else if(method.contains("Visa - 1111")) {	
				BrowserAction.click(PayNowPageObjectMap.PAY_NOW_PAGE_INVOICE_PAYMENT_SELECT_PAYMENT_METHOD_CHANGE_BUTTON_XPATH);
				BrowserWait.waitUntilElementIsDisplayed(PayNowPageObjectMap.PAY_NOW_PAGE_SELECT_YOUR_PAYMENT_METHOD_CONTAINER_APPEARING_ON_CLICKING_CHANGE_XPATH);
				List<WebElement> thElements = BrowserAction.getElements(PayNowPageObjectMap.PAY_NOW_PAGE_PAYMENT_METHOD_ROWS_APPEARING_ON_CLICKING_CHANGE_XPATH);
				int index=0;
	...
	@Test
	@Documentation(step = "Select Paypal as payment method, when it's not already added to account.", expected = "Able to Select Paypal.")
	public static void selectAddPaypalTest() throws Exception {
		ScriptLogger.info();
		try {
			String method = BrowserAction.getElement(PayNowPageObjectMap.PAY_NOW_PAGE_SELECT_A_PAYMENT_SELECTED_METHOD_NAME_XPATH).getText();
			switch (method) {
			...
			case "American Express - 8431":
				...
				int index = 0;
				for(WebElement el : thElements) {
					if(el.findElement(By.className("avatar")).findElement(By.tagName("img")).getAttribute("title").equals("New PayPay Account")) {
						break;
					} else {
						index++;
					}
				...
				thElements.get(index).findElement(By.id("selectNewPM")).click();
				break;
				
			case "gurucomdev_per@gmail.com":	
				ScriptLogger.debug("Paypal already added into the account.");
				break;
			...
			throw new ScriptException("Unable to select paypal.");
		}
	}
	
	@Test
	@Documentation(step = "Verify Paypal is shown as pay option by default, being the primary method.", expected = "Able to verify.")
	public static void verifyPaypalIsPrimaryTest() throws Exception {
		ScriptLogger.info();
		String method = null;
		try {
			 method = BrowserAction.getElement(PayNowPageObjectMap.PAY_NOW_PAGE_SELECT_A_PAYMENT_SELECTED_METHOD_NAME_XPATH).getText();
		} catch (Exception e) {
			throw new ScriptException("Unable to get Pay method name.");
		}
		if(!method.equals("gurucomdev_per@gmail.com")) {
			throw new ApplicationException("Paypal wasn't the primary/default pay method appearing.");
		}
	}
