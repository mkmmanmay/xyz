...
public class ManagePageTest {
	...
	@Test
	@Documentation(step = "click dropdown toggle on first Consultant role appearing", expected = "Able to verify.")
	public static void clickFirstConsultantDropdownTest() throws Exception {
		ScriptLogger.info();
		try {
			List<WebElement> roles = BrowserAction.getElements(ManagePageObjectMap.MANAGE_PAGE_MY_MANAGERS_SUB_TAB_MEMBER_TILES_TEXT_XPATH);
			int index = 0;
			for (int i = 0; i < roles.size(); i++) {
				if(roles.get(i).findElement(By.cssSelector(".memberRole")).getText().equals("Consultant")) {
					WebDriverAction.hoverOverElement(roles.get(i), WebDriverAction.ACTION_STYLE_WEBDRIVER);
					Thread.sleep(2000);
					roles.get(i).findElement(By.xpath("//a[@class='module_btn secondary_btn dropdown-toggle']")).click();
					break;
			...
			throw new ApplicationException("Unable to click; possible locator or selenium issue. If not, possible application issue. Or perhaps the elements after clicking didn't appear.")	;	
		}
	}
	...
	public static void clickRemoveInDropdownTest() throws Exception {
		...
		try {
			...
			throw new ApplicationException("Unable to click on 'Remove'.")	;	
		}
	}
	...
	// -------------------- MANAGE TEAM PAGE FREELANCER ----------------
	// ------------------------------ START ----------------------------
	@Test
	@Documentation(step = "Verify Manage Team page on Freelancer. side", expected = "Able to verify.")
	public static void verifyFLManageTeamPageTest() throws Exception {
		ScriptLogger.info();
		try {
			BrowserWait.waitForPageToBeLoaded();
			BrowserWait.waitUntilPageTitle("View Team Members - Guru");
			BrowserWait.waitUntilText("Team Members", 30);
			BrowserWait.waitUntilElementIsDisplayed(ManagePageObjectMap.MANAGE_PAGE_FL_ADD_TEAM_MEMBER_BUTTON_ID);
			BrowserWait.waitUntilText("Collaborative Team Members");
			BrowserWait.waitUntilText("Managers");
			BrowserWait.waitUntilText("Consultants");
			BrowserWait.waitUntilText("Billable Only");
		} catch (Exception e) {
			throw new HTMLElementNotFoundException(e, "One of more element necessary for Manage team page on FL side verification didn't appear.")	;	
		}
	}
