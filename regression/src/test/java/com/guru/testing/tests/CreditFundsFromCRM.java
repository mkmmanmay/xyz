...
public class CreditFundsFromCRM {
	...
	@Test(dependsOnMethods="navigateToCreditDebitTest")
	@Parameters({"userType"})
	@Documentation(step="Click on 'Credit ____ account', where the ____ is either Pro or Emp.", expected="Able to click link based on user type.")
	public static void goToCreditOptionTest(String userType) throws Exception{
		AdminCRMPageTest.clickCreditAccountTest(userType);
	}
	
	@Test(dependsOnMethods="goToCreditOptionTest")
	@Parameters({"creditAmount", "wireNumber"})
	@Documentation(step="Enter Amount to credit, and a note against the credit.", expected="Able to enter the information.")
	public static void enterAmountDetailsTest(String creditAmount, String wire) throws Exception{
		AdminCRMPageTest.enterAmountToCreditTest(creditAmount);
		AdminCRMPageTest.enterWireNumberTest(wire);
	}
	
	@Test(dependsOnMethods="enterAmountDetailsTest")
	@Documentation(step="Click 'Accept', and verify the payment was successful.", expected="Credit account should be successful.")
	public static void clickAcceptTest() throws Exception{
		AdminCRMPageTest.clickAcceptBtnTest();
	}
}
