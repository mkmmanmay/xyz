...
public class JobDetailsPageTest {
	...
	public static void enterQuestionTest(String askQuestion) throws Exception {
		ScriptLogger.info();
		try {
			BrowserAction.clickAndClear(JobDetailsPageObjectMap.JOB_DETAILS_PAGE_ASK_QUESTION_TEXT_AREA_ID);
			BrowserWait.waitUntilElementIsDisplayed(JobDetailsPageObjectMap.JOB_DETAILS_PAGE_ASK_QUESTION_TEXT_AREA_FOCUSED_XPATH,10);
		} catch (Exception e) {
			throw new ApplicationException("Ask question area is not enabled");
		}
		try{
			BrowserAction.enterFieldValue(JobDetailsPageObjectMap.JOB_DETAILS_PAGE_ASK_QUESTION_TEXT_AREA_ID,askQuestion);
		} catch (Exception e) {
	...
	@Test
	@Documentation(step = "Verfify Ask question is enabled and click Ask Question ", expected = "Ask Question button should be clicked")
	public static void clickAskQuestionTest() throws Exception {
		ScriptLogger.info();		
		try {
			try {
				BrowserWait.waitUntilElementIsNotDisplayed(JobDetailsPageObjectMap.JOB_DETAILS_PAGE_ASK_QUESTION_BUTTON_DISABLED_XPATH);
			} catch (Exception e) {
				throw new ApplicationException(e,"Ask Question button is still enabled");
			}
			BrowserAction.click(JobDetailsPageObjectMap.JOB_DETAILS_PAGE_ASK_QUESTION_BUTTON_ENABLED_XPATH);
		} catch (Exception e) {
			throw new ScriptException(e);
		}
	}
	...
	@Test
	@Parameters("askQuestion")
	@Documentation(step = "Verify the latest question is posted", expected = "It should be verified")
	public static void verifyQuestionAskedTest(String askQuestion) throws Exception {
		ScriptLogger.info();
		List <WebElement> messages;
		String messageRecieved;
		try {
			messages=BrowserAction.getElements(JobDetailsPageObjectMap.JOB_DETAILS_PAGE_ASK_QUESTION_MESSAGE_THREAD_XPATH);
		} catch (Exception e) {
			throw new ScriptException(e);
		}
			if(messages.size()>1){
				messageRecieved=messages.get(messages.size()-1).getText();
			}
			else if (messages.size()==1) {
				messageRecieved=messages.get(0).getText();
			}
			else {
				throw new ApplicationException("Message thread is not found");
			}
			if(!askQuestion.equals(messageRecieved)){
				throw new ApplicationException("Message Expected: "+askQuestion+"\nMessage Actual:"+messageRecieved);
			}
		
	}
	@Test
	@Parameters("askQuestion")
	@Documentation(step = "Enter a question", expected = "Able to select options from the dropdown list.")
	public static void enterQuestion2Test(String askQuestion) throws Exception {
		enterQuestionTest(askQuestion+" - This question should be deleted");
		
	}
	
	@Test
	@Documentation(step = "Click 'Delete' icon and Click on Ok on alert", expected = "It should be done")
	public static void deleteQuestionTest() throws Exception {
		ScriptLogger.info();
		try {
			BrowserAction.click(JobDetailsPageObjectMap.JOB_DETAILS_PAGE_ASK_QUESTION_DELETE_ICON_XPATH);
	...
	public static void verifyQuestionDeletedTest() throws Exception {
		ScriptLogger.info();
		try {
	...
	public static void verifyPortfolioIsPresentTest(String portfolioName) throws Exception {
		ScriptLogger.info();
		try {
				WebElement element= WebDriverAction.getElement(By.id("listWork"));
				List <WebElement> portfolioList=element.findElements(By.tagName("li"));
				String xpathExpression="//div[@class='title ng-binding'][contains(text(),'"+portfolioName+"')]";
				for (WebElement webElement : portfolioList) {
					webElement.findElement(By.xpath(xpathExpression));
				}
		} catch (Exception e) {
			throw new ScriptException(e);
		}
	...
	@Test
	@Documentation(step = "Click To submit the quote", expected = "Quote should be submitted")
