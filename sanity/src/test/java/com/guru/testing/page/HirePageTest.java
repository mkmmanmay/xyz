...
public class HirePageTest {
	...
	public static void verifyHirePageTest() throws Exception {
		ScriptLogger.info();
		try {			
			BrowserWait.waitUntilElementIsDisplayed(HirePageObjectMap.HIRE_PAGE_QUOTES_TAB_XPATH);
			...
			throw new HTMLElementNotFoundException(e,"Emp Quotes Account page elements not loaded");
		}
	}
	...
	@Test
	@Parameters("hireIndex")
	@Documentation(step = "", expected = "")
	public static void clickHireInQuotesByIndexTest(int hireIndex) throws Exception {
		ScriptLogger.info();
		try {
			WebElement element= BrowserAction.getElements(HirePageObjectMap.HIRE_PAGE_HIRE_FL_BUTTON_XPATH).get(0);
			element.click();
	...
	public static void verifyMilestoneAgreementPopUpTest() throws Exception {
		...
		try {		
				BrowserWait.waitUntilText("Work Room","will be placed in the Work Room: ","Do you want to use the agreement submitted with the quote?");
				BrowserWait.waitUntilText("Yes - approve the agreement","No - request that the guru revises agreement","By checking this box, I acknowledge that:");
				BrowserWait.waitUntilText("This agreement is between","Guru.com is not an agent of");
				BrowserWait.waitUntilElementIsDisplayed(HirePageObjectMap.HIRE_PAGE_POP_UP_HIRE_BUTTON_DISABLED_XPATH);
