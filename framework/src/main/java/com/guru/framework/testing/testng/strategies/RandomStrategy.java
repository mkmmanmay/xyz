...
import com.guru.framework.testing.utils.objects.iterator.RandomIterator;
...
public class RandomStrategy extends AbstractDataTableStrategy {
	
	protected RandomStrategy(final DataTable table) {
		this(table, table.getRowCount());
	}
	
	protected RandomStrategy(final DataTable table, final int limit) {
		this.iterator = new RandomIterator(table, limit);
	}
	
	public RandomStrategy(final Map<String, List<String>> data) throws DataTableException {
		this(buildTableFromMap(data));
	}
	
	public RandomStrategy(final Map<String, List<String>> data, final int limit) throws DataTableException {
	...
	public static void main(final String...args) throws DataTableException {
		...
		map.put("1", Arrays.asList("11", "12", "13", "14", "15"));
		map.put("2", Arrays.asList("21"));
		map.put("3", Arrays.asList("31", "32"));
		
		final RandomStrategy ps = new RandomStrategy(map, 5);
