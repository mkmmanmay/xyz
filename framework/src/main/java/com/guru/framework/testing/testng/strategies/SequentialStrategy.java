...
public class SequentialStrategy extends AbstractDataTableStrategy {

		
	protected SequentialStrategy(final DataTable table) {
		this(table, table.getRowCount());
	}
	
	protected SequentialStrategy(final DataTable table, final int limit) {
		this.iterator = new CyclicalIterator(table, limit);
	}
	
	public SequentialStrategy(final Map<String, List<String>> data) throws DataTableException {
		this(buildTableFromMap(data));
	}
	
	public SequentialStrategy(final Map<String, List<String>> data, final int limit) throws DataTableException {
	...
	public static void main(final String...args) throws DataTableException {
		...
		map.put("1", Arrays.asList("11", "12", "13", "14"));
	//	map.put("2", Arrays.asList("21"));
	//	map.put("3", Arrays.asList("31", "32"));
		
		final SequentialStrategy ps = new SequentialStrategy(map);
